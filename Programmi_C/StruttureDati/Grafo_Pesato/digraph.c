...
diwegraph wegraph_new(int max_size){
    struct _wegraph* g = (struct _wegraph*)malloc(sizeof(struct _wegraph));
    if (g != NULL){
...
void wegraph_destroy(wegraph g){
    free(g->nodes);
...
int diwegraph_add_node(wegraph g, int node_value){
...
int wegraph_add_edge(wegraph g, int node1_id, int node2_id, float weight){
    ...
    g->weights[node1_id][node2_id] = weight;
    return 0;
}
...
bool wegraph_weight(wegraph g, int node1, int node2, float* result){
    if(!dgraph_adjacent(h, node1,node2)){
        return false;
    }
    return g->weights[node1][node2] = result;
}
...
void wegraph_bfs(wegraph g, int start_node){
    ...
    while(!queue_is_empty(q)){
        ...
        if(!visited[node_to_analyze]){
            ...
            int n = wegraph_neighbors(g, node_to_analyze, neighbors);
            for (int i = 0; i < n; i++){
...
void wegraph_dfs_rec(wegraph g, int start_node, bool* visited){
    ...
    int neighbors[g->size];
    int n = wegraph_neighbors(g, start_node, neighbors);
    visited[start_node] = true;
    for (int i = 0; i < n; i++){
        wegraph_dfs_rec(g, neighbors[i], visited);
    }
}
void wegraph_dfs(wegraph g, int start_node){
    ...
    //chiamare wegraph rec
    wegraph_dfs_rec(g, start_node, visited);
}
...
//funzione che mi restituisce il peso del percorso tra due nodi
float wegraph_path_cost_rec(wegraph g, int node1, int node2, float *weight){
